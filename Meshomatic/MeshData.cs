...
namespace Meshomatic {
	...
	public class MeshData {
        ...
        public double[] VertexArrayDouble;
        public float[] VertexArrayFloat;
        public float[] NormalArrayFloat;
		...
		public MeshData(Vector3[] vert, Vector3[] norm, Vector2[] tex, Tri[] tri) {
            ...
            VertexArrayDouble = ToVertexArrayDouble();
            VertexArrayFloat = ToVertexArrayFloat();
            NormalArrayFloat = ToNormalArrayFloat();
		...
		/// </returns>
		private double[] ToVertexArrayDouble() {
        ...
        private float[] ToVertexArrayFloat()
        {
            float[] verts = new float[Vertices.Length * 3];
            for (int i = 0; i < Vertices.Length; i++)
            {
                verts[i * 3] = (float)Vertices[i].X;
                verts[i * 3 + 1] = (float)Vertices[i].Y;
                verts[i * 3 + 2] = (float)Vertices[i].Z;
            }

            return verts;
        }

        private float[] ToNormalArrayFloat()
        {
            float[] norms = new float[Normals.Length * 3];
            for (int i = 0; i < Normals.Length; i++)
            {
                norms[i * 3] = (float)Normals[i].X;
                norms[i * 3 + 1] = (float)Normals[i].Y;
                norms[i * 3 + 2] = (float)Normals[i].Z;
            }
