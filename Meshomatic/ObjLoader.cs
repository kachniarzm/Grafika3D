...
namespace Meshomatic
{
    public class ObjLoader
    {
        public MeshData LoadStream(Stream stream)
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                switch (parameters[0])
                {
                    ...
                    case "f":
                        // Face
                        tris.AddRange(parseFace(parameters, points.Count, normals.Count));
                        break;
                }
            }
        ...
        private static Tri[] parseFace(string[] indices, int pointsNr, int normalNr)
        {
            ...
            return Triangulate(p, pointsNr, normalNr);
            //return new Face(p);
        }

        // Takes an array of points and returns an array of triangles.
        // The points form an arbitrary polygon.
        private static Tri[] Triangulate(Point[] ps, int pointsNr, int normalNr)
        {
            Point[] ps_recalculated = RecalculatePoint(ps, pointsNr, normalNr);

            List<Tri> ts = new List<Tri>();
            if (ps_recalculated.Length < 3)
            {
                throw new Exception("Invalid shape!  Must have >2 points");
            }

            Point lastButOne = ps_recalculated[1];
            Point lastButTwo = ps_recalculated[0];
            for (int i = 2; i < ps_recalculated.Length; i++)
            {
                Tri t = new Tri(lastButTwo, lastButOne, ps_recalculated[i]);
                lastButOne = ps_recalculated[i];
                //lastButTwo = ps_recalculated[i - 1];
                ts.Add(t);
            }
            return ts.ToArray();
        }

        private static Point[] RecalculatePoint(Point[] ps, int pointsNr, int normalNr)
        {
            ...
            for (int i=0; i<result.Length; i++)
            {
                ...
                {
                    normal = normalNr + ps[i].Normal + 1;
                }

                result[i] = new Point(vertex, normal, ps[i].TexCoord);
            }
